...
pyinstaller --name="K-Matrix Toolkit" --add-data "images:images" --add-data "src:src" --windowed --icon=images/porsche-model-gt3rs-logo.icns --target-arch x86_64 --noconfirm K_Matrix_Tool_APP.py
mkdir -p dist/dmg
